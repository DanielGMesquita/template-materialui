...
const ListarTarefa = () => {
  ...
  const handleDeletar = (id) => {
    setTarefas(currentTarefas => {
      ...
      const novasTarefas = currentTarefas.filter(tarefa => tarefa.idTarefa !== id);
      
      // Verifica se houve mudança nas tarefas
      if (JSON.stringify(novasTarefas) !== JSON.stringify(currentTarefas)) {
        return novasTarefas; // Retorna o novo estado apenas se houver mudança
      } else {
        return currentTarefas; // Retorna o estado atual se não houver mudança
      }
    });
  };
        ...
        <CardContent>
            ...
            <Table sx={{ minWidth: 650 }} size="small" aria-label="a dense table">
                ...
                <TableBody>
                  {tarefas.map((row) => (
                    <TableRow
                      key={row.idTarefa} // Usando o ID da tarefa como chave
